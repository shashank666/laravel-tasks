...
<div class="card my-2 card-post" id="opinion_{{$opinion->id}}">

 
    <div class="card-header opinion-header" style="border-top-left-radius: 10px;border-top-right-radius:10px">
            <div class="media align-items-center">
                <a href="{{ route('user_profile', ['username' => $user['username']]) }}" data-placement="right" title="Go to the profile of {{ucfirst($user['name'])}}"><img class="rounded-circle lazy" src="/img/avatar_thumb.png" data-src="
    @php
    ...
    @endphp" height="40" width="40" alt="{{ucfirst($user['name'])}}" onerror="this.onerror=null;this.src='/img/avatar_thumb.png';"/></a>
                 <div class="media-body">
                    <div class="d-flex justify-content-between align-items-center w-100">
                         <div class="content">
                         <span class="ml-2 user_{{$index}} viewinfo" data-cardshow="{{$index}}" title='Please wait..' id="user_{{$user['id']}}"><a href="{{ route('user_profile', ['username' => $user['username']]) }}" style="color:black;">{{ucfirst($user['name'])}}</a><br/>
						 <span style="font-size:12px;color:#666666" class="viewinfo viewinfo-sec">{{'@'.$user['username']}}</span></span>
                     </div>
                         <span class="text-secondary" style="cursor:default;font-size:12px">
    ...
    <div class="card-body opinion-body" style="overflow-x:hidden;overflow-y:hidden; cursor:pointer">
        <p class="desc" onclick="window.location.href='{{ '/@'.$opinion->user['username'].'/opinion/'.$opinion->uuid}}'">{!!nl2br($opinion->body)!!}</p>
        ...
        <div class="row">
            <div class="col-12">
                ...
                @else
                    ...
                    @else
                        <img class="img-fluid rounded lazy" src="/img/noimg_320x240.png" data-src="{{preg_replace('/.[^.]*$/', '',$opinion->cover).'_314x240'.'.'.$ext_opinion}}" height="auto" width="auto" alt="{{str_replace('#',' ',$opinion->hash_tags)}}"/>
                    @endif
     ...
     <div class="card-footer opinion-footer" style="border-bottom-left-radius: 10px;border-bottom-right-radius:10px">

            {{-- Like & Comment --}}
            <div class="like_comment_div">
                @if(Auth::guest())

                <span class="feed_card_action_item">
                <span class="like_guest" data-placement="top" title="Please Login To Like"><i class="far fa-arrow-alt-circle-up mr-1 like-icon-opined"></i>
                        ...
                        <span class="align-top like-icon-text" style="margin-right:15px;">{{$opinion->AgreeCount}} Agrees</span>
                        @else
                        <span class="align-top like-icon-text" style="margin-right:15px;">{{$opinion->AgreeCount}} Agree</span>
                        @endif
                </span>
                </span>
                ...
                <span class="dislike_guest" data-placement="top" title="Please Login To Disagree"><i class="far fa-arrow-alt-circle-down mr-1 like-icon-opined"></i>
                        ...
                        <span class="align-top like-icon-text" style="margin-right:15px;">{{$opinion->DisagreeCount}} Disagrees</span>
                        @else
                        <span class="align-top like-icon-text" style="margin-right:15px;">{{$opinion->DisagreeCount}} Disagree</span>
                        @endif
                </span>
                </span>
                
                <span class="feed_card_action_item">
                <span class="mr-4 showComment" data-opinion="{{$opinion->id}}"  data-placement="top" title="Please Login To Comment"  onclick="window.location.href='{{ '/@'.$opinion->user['username'].'/opinion/'.$opinion->uuid}}'"><i class="far fa-comments mr-1 comment-icon-opined"></i>
                ...
                {{-- Agree Part------ --}}
                    ...
                    <span id="like_{{$opinion->id}}" class="like" >
                        ...
                        <i class="fas fa-arrow-alt-circle-up mr-1 like-icon-opined"></i>
                        </span>
                    </span>
                    {{-- Agree Text --}}
                     @if($opinion->AgreeCount>1)
                        <span class="align-top like-icon-text btn-opinion-likes-count" style="cursor: grab;margin-right:10px;" id="agree_count_{{$opinion->id}}" data-opinion="{{$opinion->id}}"  data-placement="top" title="See Likes">{{$opinion->AgreeCount}} Agrees</span>
                        @else
                        <span class="align-top like-icon-text btn-opinion-likes-count" style="cursor: grab;margin-right:10px;" id="agree_count_{{$opinion->id}}" data-opinion="{{$opinion->id}}"  data-placement="top" title="See Likes">{{$opinion->AgreeCount}} Agree</span>
                     @endif
                    </span>
                ...
                {{-- Disagree Part----- --}}
                    ...
                    {{-- DisAgree Text --}}
                     @if($opinion->DisagreeCount>1)
                        <span class="align-top like-icon-text btn-opinion-likes-count" style="cursor: grab;margin-right:10px;" id="disagree_count_{{$opinion->id}}" data-opinion="{{$opinion->id}}"  data-placement="top" title="See DisLikes">{{$opinion->DisagreeCount}}  Disagrees</span>
                     @else
                        <span class="align-top like-icon-text btn-opinion-likes-count" style="cursor: grab;margin-right:10px;" id="disagree_count_{{$opinion->id}}" data-opinion="{{$opinion->id}}"  data-placement="top" title="See DisLikes">{{$opinion->DisagreeCount}} Disagree</span>
                     ...
                     {{-- Comments Icon --}}
                     <span class="feed_card_action_item">
                    <span class="comment mr-4 showComment" data-opinion="{{$opinion->id}}" data-placement="top" title="Comment"  onclick="window.location.href='{{ '/@'.$opinion->user['username'].'/opinion/'.$opinion->uuid}}'">
                ...
                @endif
        <span class="dropdown-toggle share-opinion feed_card_action_item" id="share-{{$opinion->id}}" data-toggle="dropdown"  style="font-size:18px;" aria-haspopup="true" aria-expanded="false" name = "test">
            <i class="fas fa-share mr-1"  data-placement="top" title="Share this Opinion"></i>{{$opinion->sharesCount}}
        </span>
    ...
    <span class="feed_card_action_item">
        ...
        <span id="delete_{{$opinion->id}}" class=" ml-3 delete text-danger"  data-placement="top" title="Delete Opinion">
