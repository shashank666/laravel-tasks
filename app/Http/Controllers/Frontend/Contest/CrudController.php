...
use App\Model\ShortOpinionDisagree;
...
use App\Mail\Post\PostInformMail;
                                    ...
                                    '<div class="d-flex justify-content-between align-items-center w-100">'.
                                            ...
                                            if($post_likes[$i]->user["id"]!=Auth::user()->id){
                                                if(!in_array($post_likes[$i]->user["id"],$followingids)){
                                                    $middle='<button data-userid="'.$post_likes[$i]->user["id"].'" class="followbtn followbtn_'.$post_likes[$i]->user["id"].' btn btn-sm btn-outline-success" style="display:block">Follow <span><i class="fas fa-user-plus ml-2"></i><span></button>'.
                                                    ...
                                                    $middle='<button  data-userid="'.$post_likes[$i]->user["id"].'" class="followbtn followbtn_'.$post_likes[$i]->user["id"].' btn btn-sm btn-outline-success" style="display:none">Follow <span><i class="fas fa-user-plus ml-2"></i><span></button>'.
...
*/
    ...
    public function publish(Request $request){
       ...
       DB::table('post_keywords')->where('post_id', '=', $post->id)->update(['is_active' => 1]);
        $opinion=ShortOpinion::where('post_id', '=', $post->id)->first();
                        if($opinion)
                        {
                            ...
                            foreach($opinion_comments as $op_comment){
                                ...
                                DB::table('short_opinion_comments_disagree')->where('comment_id',$op_comment->id)->update(['is_active'=>1]);
                            }
        ...
        try{
           //Mail::send(new PostInformMail($user,$post));
    ...
    {
        $post=Post::where(['slug'=>$slug,'is_active'=>1])->with('categories:category_id','threads:name')->first();
        if($post){
    ...
    public function update(Request $request)
    {
     ...
     public function show($slug,Request $request)
     {
        ...
        $contest=Contest::where(['slug'=>$slug,'is_active'=>1])->orWhere(['slug'=>$new_slug])->first();

        if($contest){
             $ip_address=isset($_SERVER["HTTP_CF_CONNECTING_IP"])?$_SERVER["HTTP_CF_CONNECTING_IP"]:$request->ip();
             
            // $hash_tags=Contest::select('hash_tags')->where(['is_active'=>1])->orderBy('created_at','desc')->take(6)->get();
            $description=Contest::select('description')->where('is_active',1)->orderBy('created_at','desc')->take(6)->get();
            // $contest_board=ShortOpinion::where(['hash_tags'=> [$hash_tag]])->select('plain_body',[$description])
            // ->selectSub(function ($q) {
            //     $q->from('short_opinion_likes')
            //       ->whereRaw('short_opinion_likes.short_opinion_id = short_opinions.id')
            //       ->where('is_active', 1)
            //       ->select('count(*)');
            //   }, 'likes_count')
            ...
            $hash_tags=preg_split("/[\s,]+/", $contest->hash_tags);
            
            // foreach($hash_tags as $tag){
                $threadIds=[];
                foreach($hash_tags as $tag){
                    $thread=Thread::where(['name'=>$tag,'is_active'=>1])->first();
                    if($thread){
            ...
            // $thread_opinions=ThreadOpinion::where(['thread_id'=>$thread->id])->orderBy('created_at','desc')->paginate(12);
              $contest_board = ThreadOpinion::whereIn('thread_opinions.thread_id',$threadIds)
              ...
              foreach($contest_board as $trending_thread_opinion)
                {  
                  $trending_thread_opinion=ShortOpinion::where(['id'=>$trending_thread_opinion->short_opinion_id,'is_active'=>1,'cover_type'=>$contest->contest_type,'type'=>$contest_category])->with('user')->first();
            ...
            //   {
            //     $opinion=ShortOpinion::where(['id'=>$opinion->short_opinion_id,'is_active'=>1,'cover_type'=>$contest_type])->with('user')->first();
            //     if($opinion){
            //         array_push($opinions,$opinion);
            //     }
            //   }
                // $view = (String) view('frontend.contest.crud.read',compact('google_ad1','contest','contest_board','description','hash_tag'));
                // return response()->json(['html'=>$view]);
            ///
            
              

             return view('frontend.contest.crud.read',compact('google_ad1','contest','contest_board','opinions','hash_tags','contest_type','trending_opinions','thread'));
        }else{
             abort(404);
         }
     }
                ...
                if($opinion){
                   ...
                   $opinion_comments=DB::table('short_opinion_comments')->where('short_opinion_id',$opinion->id)->get();
                    foreach($opinion_comments as $op_comment){
                         ...
                         DB::table('short_opinion_comments_disagree')->where('comment_id',$op_comment->id)->update(['is_active'=>$is_active]);
                    }
...
// public function leaderboard_details() {
//     $hash_tag=Contest::select('hash_tag')->where('is_active',1)->orderBy('created_at','desc')->take(6)->get();
//     $description=Contest::select('description')->where('is_active',1)->orderBy('created_at','desc')->take(6)->get();
//     $contest_board=ShortOpinion::where(['hash_tags'=> [$hash_tag]])->select('plain_body',[$description])
//     ->selectSub(function ($q) {
//         $q->from('short_opinion_likes')
...
//       }, 'likes_count')
//       ->selectSub(function ($q) {
//         $q->from('short_opinion_comments')
//           ->whereRaw('short_opinion_comments.short_opinion_id = short_opinions.id')
//           ->where('is_active', 1)
//           ->selectRaw('count(*)');
//       }, 'comments_count')
//       ->orderBy('comments_count','likes_count', 'desc')
//       ->take(50)
//       ->get();

     
//         return view('frontend.contest.crud.read',compact('contest_board','comments_count','likes_count'));
