...
class OpinionCrudController extends Controller
{
    ...
    public function index(Request $request){
        ...
        // }
        $user_id_test = rand(4606,4805);
        //$user_id_test = rand(60,65);
        ...
        if($request->ajax()){
          $view = (String) view('frontend.opinions.components.opinions_loop');
          return response()->json(['html'=>$view]);
        }else{
            return view('admin.dashboard.opinion.write_opinion');
        }
    }
    ...
    public function store(Request $request)
    {
      ...
      $user_test = User::whereBetween('id',[4406,4905])->inRandomOrder()->first();
        // if($admin_id==4){
        //     if($gender_op!=null){
        //         $user_test = User::where(['gender'=>$gender_op])->whereBetween('id',[4406,4605])->inRandomOrder()->first();
        //     }
        //     else{
        //         $user_test = User::whereBetween('id',[4406,4605])->inRandomOrder()->first();
        //     }
        //   //$user_id_test = rand(4406,4505);
        // }
        // elseif($admin_id==2){
        //   if($gender_op!=null){
        //         $user_test = User::where(['gender'=>$gender_op])->whereBetween('id',[4506,4605])->inRandomOrder()->first();
        //     }
        //     else{

        //         $user_test = User::whereBetween('id',[4506,4605])->inRandomOrder()->first();
        //     }
        // }
        // elseif($admin_id==7){
        //     if($gender_op!=null){
        //         $user_test = User::where(['gender'=>$gender_op])->whereBetween('id',[4606,4705])->inRandomOrder()->first();
        //     }
        //     else{
        //         $user_test = User::whereBetween('id',[4606,4705])->inRandomOrder()->first();
        //     }
    ...
    $constraint->aspectRatio();
        ...
        $this->notify_followers($opinion,'ShortOpinionCreated',$user_test->id);
        return redirect()->back();
        
    }


   protected function notify_followers($object,$event,$user_id){

        $user_test = User::where(['id'=>$user_id])->first();
        $followers=$user_test->active_followers;
        ...
        try{
            if($event=='ShortOpinionCreated'){
                foreach(array_chunk($fcm_tokens,100) as $chunk){
                    dispatch(new ShortOpinionCreatedJob($object,$user_test,$chunk));
                }
                foreach($followers as $follower){
                    Notification::send($follower,new ShortOpinionCreated($object,$user_test,$fcm_tokens));
                }
            }
        }catch(\Exception $e){}
