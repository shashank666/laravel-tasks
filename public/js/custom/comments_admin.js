...
function loadOpinionCommentReplies(appendDiv,opinionID,commentID,page){
    ...
    $.ajax({
        url: `/cpanel/opinion/comments/load/replies?page=${page}&opinion_id=${opinion_id}&comment_id=${commentID}`,
...
function loadOpinionComments(opinion_id,page,event) {
    ...
    $.ajax({
        url: `/cpanel/opinion/comments/load?page=${page}&opinion_id=${opinion_id}`,
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            //$('#btnloadMore_'+opinion_id).remove();
            $('.comments_div_admin'+opinion_id).empty();
            $('.comments_div_admin'+opinion_id).append(response.html);
        },
        error: function(response) {
...
$(document).on('click','.opinionAddComment',function(){
        ...
        $('#add_opinion_comment_form').attr('action','/cpanel/opinion/comments/create');
...
$(document).on('submit', '#add_opinion_comment_form', function(e) {
    e.preventDefault();
    if($(this).attr('action')=='/cpanel/opinion/comments/create'){
        ...
        if(parentID>0){
            ...
            submitOpinionComment(opinionID,'#add_opinion_comment_form','.comments_div_admin'+opinionID,'prepend');
        }
    }else{
...
$(document).on('click', '.deleteComment', function() {
    ...
    var ask = confirm("Are You Sure? You will not able to revert this action");
    if (ask == true) {
    ...
    $.ajax({
        url: '/cpanel/opinion/comments/delete',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        type: 'post',
...
$(document).on('click', '.desableComment', function() {
    var comment_id = parseInt($(this).attr('data-commentid'));
    var opinion_id = parseInt($(this).attr('data-opinion'));
    var parent_id=parseInt($(this).attr('data-parentid'));
    $.ajax({
        url: '/cpanel/opinion/comments/desable',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        type: 'post',
        data: { comment_id: comment_id, opinion_id: opinion_id },
        dataType: 'json',
...
$(document).on('click', '.enableComment', function() {
    var comment_id = parseInt($(this).attr('data-commentid'));
    var opinion_id = parseInt($(this).attr('data-opinion'));
    var parent_id=parseInt($(this).attr('data-parentid'));
    $.ajax({
        url: '/cpanel/opinion/comments/enable',
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        type: 'post',
        data: { comment_id: comment_id, opinion_id: opinion_id },
        dataType: 'json',
...
$(document).on('click','.likeComment',function(){
    var commentID=$(this).attr('data-commentid');
        ...
        $.ajax({
        url:'/cpanel/opinion/comments/like',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type:'POST',
